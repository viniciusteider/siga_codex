		...
		if (pontos != "") {
			...
			this.directionsService.route(request, function (response, status)
			{
				if (status == google.maps.DirectionsStatus.OK) {
					...
					Squall.ToastMsg('warning',"Um ou mais pontos não podem ser encontrados.");
				}
				else if (status == google.maps.DirectionsStatus.ZERO_RESULTS) {
					Squall.ToastMsg('warning',"A rota escolhida não retornou resultado.", "Atenção!", 2);
				}
				else if (status == google.maps.DirectionsStatus.OVER_QUERY_LIMIT) {
					Squall.ToastMsg('warning',"Limite de requisições por minuto excedida.\nAguarde 60 segundos antes de fazer uma nova requisição.");
				}
				else if (status == google.maps.DirectionsStatus.REQUEST_DENIED) {
					Squall.ToastMsg('warning',"A página da web não tem permissão para utilizar o serviço de direções.", "Atenção!");
				}
				else if (status == google.maps.DirectionsStatus.UNKNOWN_ERROR) {
					Squall.ToastMsg('warning',"A solicitação de rota não pôde ser processada devido ao numero de endereços requisitados.", "Atenção!");
				}
				else {
					Squall.ToastMsg('warning',"A solicitação de rota não pôde ser processada devido a um erro no servidor.\nO pedido pode ter sucesso se você tentar novamente.");
				}
			});
		}
...
SquallGoogle.prototype.usePixelOffset = function (p1, p2)
{
...
SquallGoogle.prototype.addIcon = function (file)
{
	var g    = google.maps;
	var icon = new g.MarkerImage("assets/media/google/" + file,
		new g.Size(24, 24), null, new g.Point(12, 12));
	return icon;
};

// Creates markers with corresponding triangle icons
SquallGoogle.prototype.create = function (p1, p2, mode)
{
	this.markerpos;
	var cor = '';
	var g   = google.maps;
...
// Creates markers with corresponding triangle icons
SquallGoogle.prototype.createMult = function (p1, p2, mode, rota)
{
	this.markerpos;
	var cor = '';
	var g   = google.maps;
...
SquallGoogle.prototype.load     = function (points, mode)
{
...
SquallGoogle.prototype.loadMult = function (points, mode, rota)
{
...
SquallGoogle.prototype.carregarLinhasParalelas = function (points, color, weight, opacity, gapPx)
{
	this.gapPx                      = gapPx;
	this.points                     = points;
	this.color                      = color;
...
SquallGoogle.prototype.montarLinhas = function ()
{
	this.map.setZoom(15);
	this.setProjection();
	this.onRemove();
...
SquallGoogle.prototype.setProjection = function ()
{
...
SquallGoogle.prototype.onRemove = function ()
{
	if (this.cerca_rota) {
...
SquallGoogle.prototype.draww    = function (map)
{
	if (this.cerca_rota) {
		this.cerca_rota.setMap(null);
		this.cerca_rota = null;
...
SquallGoogle.prototype.redraw = function (force)
{
	return; //do nothing
}

SquallGoogle.prototype.recalc = function ()
{
...
SquallGoogle.prototype.intersect = function (p0, p1, p2, p3)
{
