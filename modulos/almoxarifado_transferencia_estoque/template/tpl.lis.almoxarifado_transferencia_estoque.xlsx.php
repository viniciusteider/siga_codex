...
$listar = $objAlmoxarifadoTransferenciaEstoque->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Estoque_atual];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Data_movimento];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Quantidade_tranferir];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["data_movimento"], $_SESSION["usuario"]["id_fuso_horario"]),"class"=> "uppercase"];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["quantidade_tranferir"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Almoxarifado_transferencia_estoque;
//$filtro[ROTULO_RELATORIO] = Almoxarifado_transferencia_estoque;
...
header("Content-Disposition: attachment;filename=".Almoxarifado_transferencia_estoque.".xls");
