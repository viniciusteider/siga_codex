...
$listar = $objPessoalDispensas->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Motivo];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["motivo"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Pessoal_dispensas;
//$filtro[ROTULO_RELATORIO] = Pessoal_dispensas;
...
header("Content-Disposition: attachment;filename=".Pessoal_dispensas.".xls");
