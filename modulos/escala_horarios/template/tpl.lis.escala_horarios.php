<?php
include_once("modulos/escala_horarios/template/js.lis.escala_horarios.php");
$_SESSION["FILTRO_ESCALA_HORARIOS"] = $_REQUEST;
...
$listar = $objEscalaHorarios->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_escala_horarios";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "INÍCIO","filtro"=> "hora_inicio", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "FIM","filtro"=> "hora_fim", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
	    ...
	    $cor = ($linha["cor"] != "") ? '<div class="symbol symbol-40px me-4"><div class="symbol-label fs-2 " style="background-color: '.$linha["cor"].'">&nbsp;</div></div>' :'' ;
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $cor];
...
$grid->funcao_atualizar = "AtualizarGridEscalaHorarios";
$grid->funcao_modificar = "ModificarEscalaHorarios";
$grid->valor_campo_busca = $busca;
$grid->filtro = $filtro;
$grid->id_botao_adicionar = "AdicionarRegistroEscalaHorarios";
$grid->id_botao_excluir = "ExcluirRegistroEscalaHorarios";
$grid->id_checkbox_master = "master_EscalaHorarios";
$grid->nome_lista_checkbox = "lista_EscalaHorarios";
$grid->pagina = $pagina;
