...
$listar = $objFipeModelos->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Codigo_modelo];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Codigo_fipe];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["codigo_modelo"],"class"=> "uppercase"];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["codigo_fipe"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Fipe_modelos;
//$filtro[ROTULO_RELATORIO] = Fipe_modelos;
...
header("Content-Disposition: attachment;filename=".Fipe_modelos.".xls");
