...
class ProdutosCautela
{
	...
	public function Adicionar()
	{
		$pdo = $this->getConexao();
		$sql = '
		INSERT INTO produtos_cautela SET data_hora_cadastro = UTC_TIMESTAMP() 
 ';
	...
	public function Remover($lista)
	{
		$pdo = $this->getConexao();
		$lista = implode(",",$lista);
		//$sql = "DELETE FROM produtos_cautela WHERE id IN({$lista})";
		$sql = "UPDATE produtos_cautela SET excluido = UTC_TIMESTAMP() WHERE id IN({$lista})";
		$stmt = $pdo->prepare($sql);
		return $stmt->execute();
	}
	...
	public function ListarPaginacao($param)
	{
		...
		$where = "
			WHERE produtos_cautela.excluido IS NULL
		";
		...
		if($param["busca"] != "") $where .= " AND (nome LIKE :busca)";
		 if (($param['data_hora_inicio']))  $where .= " AND produtos_cautela.data_hora_cadastro >='{$param['data_hora_inicio']}' AND produtos_cautela.data_hora_cadastro <= '{$param['data_hora_fim']}'";
		
		$sql = "
			SELECT COUNT(*) AS total
			FROM produtos_cautela
			$joins
			$where
		";
		...
		if($filtro != "") $sql .=" ORDER BY {$param['filtro']} {$param['ordem']}"; else $sql .=" ORDER BY produtos_cautela.id DESC";
	...
	{
		$pdo = $this->getConexao();
		$sql = "SELECT * FROM produtos_cautela WHERE id = :id";
		$stmt = $pdo->prepare($sql);
		$stmt->bindParam(":id",$this->id,PDO::PARAM_INT);
		$stmt->execute();
		return $stmt->fetch();
        ...
        {
            $pdo = $this->getConexao();
            $sql = "SELECT * FROM produtos_cautela WHERE excluido IS NULL";
            if($id != "") $sql .= " AND produtos_cautela.id = $id ";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        ...
        public function BuscarAutoComplete($busca){
            ...
            $sql = " SELECT
                         produtos_cautela.id,
                         produtos_cautela.nome  AS text
                     FROM produtos_cautela
                     WHERE produtos_cautela.excluido IS NULL
                     ";
    
            if($busca != ''){
                $sql .= " AND  (produtos_cautela.nome LIKE '%$busca%' OR produtos_cautela.id  LIKE '%$busca%') ";
            }
            $sql .= ' LIMIT 50';
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        ...
        public function GerarSelec($id,$nome_campo,$id_campo,$outros = '',$campo = ["id","nome"],$autocomlete = false)
        {
            ...
            $registros = $objProdutosCautela->ListarCombo($id_atual);
