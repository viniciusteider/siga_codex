<?php
include_once("modulos/almoxarifado_requisicao/template/js.lis.almoxarifado_requisicao.php");
$_SESSION["FILTRO_ALMOXARIFADO_REQUISICAO"] = $_REQUEST;
...
$listar = $objAlmoxarifadoRequisicao->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_almoxarifado_requisicao";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "NOME","filtro"=> "nome", "tipo"=> "$ordem"];
    $dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "QUANTIDADE","filtro"=> "nome", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "ITENS","filtro"=> "nome", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["nome_solicitante"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["quantidade_produtos"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["itens"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["data"], $_SESSION["usuario"]["timezone"],'d/m/y')];
...
//Componente::FiltrarRelatorioConfiguracao($dados_coluna, $dados_linha, $_SESSION["configuracao_usuario"]["almoxarifado_requisicao"]);
$grid = new GerarGrid();
$grid->form = $dados_form;
$grid->tabela = $dados_tabela;
$grid->titulo = "";
$grid->permitir_busca = false;
$grid->funcao_atualizar = "AtualizarGridAlmoxarifadoRequisicao";
$grid->funcao_modificar = "ModificarAlmoxarifadoRequisicao";
$grid->valor_campo_busca = $busca;
$grid->filtro = $filtro;
$grid->id_botao_adicionar = "AdicionarRegistroAlmoxarifadoRequisicao";
$grid->id_botao_excluir = "ExcluirRegistroAlmoxarifadoRequisicao";
$grid->id_checkbox_master = "master_AlmoxarifadoRequisicao";
$grid->nome_lista_checkbox = "lista_AlmoxarifadoRequisicao";
$grid->pagina = $pagina;
