...
$listar = $objAlmoxarifadoRequisicao->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["data"], $_SESSION["usuario"]["id_fuso_horario"]),"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Almoxarifado_requisicao;
//$filtro[ROTULO_RELATORIO] = Almoxarifado_requisicao;
...
header("Content-Disposition: attachment;filename=".Almoxarifado_requisicao.".xls");
