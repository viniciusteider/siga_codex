<?php
include_once("modulos/medicos/template/js.lis.medicos.php");
$_SESSION["FILTRO_MEDICOS"] = $_REQUEST;
...
$listar = $objMedicos->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_medicos";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "CRM","filtro"=> "crm", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "NOME","filtro"=> "nome", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "TIPO","filtro"=> "tipo_inscricao", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "SITUAÇÃO","filtro"=> "situacao_inscricao", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "ESPECIALIDADE","filtro"=> "especialidade", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "CRO CRM","filtro"=> "cro_crm", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "UF","filtro"=> "UF", "tipo"=> "$ordem"];
//$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "D","filtro"=> "data_hora_cadastro", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["tipo_inscricao"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["situacao_inscricao"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["especialidade"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["cro_crm"]];
...
$grid->funcao_atualizar = "AtualizarGridMedicos";
$grid->funcao_modificar = "ModificarMedicos";
$grid->valor_campo_busca = $busca;
$grid->filtro = $filtro;
$grid->id_botao_adicionar = "AdicionarRegistroMedicos";
$grid->id_botao_excluir = "ExcluirRegistroMedicos";
$grid->id_checkbox_master = "master_Medicos";
$grid->nome_lista_checkbox = "lista_Medicos";
$grid->pagina = $pagina;
