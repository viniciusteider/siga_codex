...
class RelatorioUniformes
{
	...
	public function GerarRelatorio($param)
	{
        $pdo = $this->getConexao();
        $sql ="SELECT
                  COUNT(uniforme_peca.nome) AS total,
                ...
                WHERE (grupo.id = {$param['id_grupo']} OR grupo.arvore LIKE '%{$param['id_grupo']}%')
                ";
        ...
        $sql .= ' GROUP BY uniforme_tamanho.nome ORDER BY nome_peca ASC';
