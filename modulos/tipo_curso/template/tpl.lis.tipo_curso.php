<?php
include_once("modulos/tipo_curso/template/js.lis.tipo_curso.php");
$_SESSION["FILTRO_TIPO_CURSO"] = $_REQUEST;
...
$objTipoCurso = new TipoCurso();
$listar = $objTipoCurso->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_tipo_curso";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "DESCRITIVO","filtro"=> "descritivo", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["descritivo"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["id"],"nome" => "Alterar","style" => "text-align:right"];
		$x++;
	}
}
//Componente::FiltrarRelatorioConfiguracao($dados_coluna, $dados_linha, $_SESSION["configuracao_usuario"]["tipo_curso"]);
$grid = new GerarGrid();
$grid->form = $dados_form;
$grid->tabela = $dados_tabela;
$grid->titulo = "";
$grid->permitir_busca = false;
$grid->funcao_atualizar = "AtualizarGridTipoCurso";
$grid->funcao_modificar = "ModificarTipoCurso";
