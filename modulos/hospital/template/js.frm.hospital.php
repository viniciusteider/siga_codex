...
$(document).ready(function () {
    $("#bt_salvar").click(function () {
        ...
        if(id != "")
        {
            url = "index_xml.php?app_modulo=hospital&app_comando=atualizar_hospital&app_codigo";
            tipo = 2;
        }
        else
        {
            url = "index_xml.php?app_modulo=hospital&app_comando=adicionar_hospital&app_codigo";
        }
...
{
	    if (Squall.ValidateForm($("#frm_hospital"))) {
    $("#bt_salvar").prop("disabled",true).html("<i class=\"fa fa-spin fa-spinner\"></i> AGUARDE...").addClass("btn-warning");
		// ao clicar em salvar enviando dados por post via AJAX
		$.post(url,
			$("#frm_hospital").serialize(),
			// pegando resposta do retorno do post
			function (response)
			{
				if (response["codigo"] == 0) {
                    Squall.ToastMsg('success',response["mensagem"],"#index_xml.php?app_modulo=hospital&app_comando=listar_hospital",'600');

                        // $("#frm_hospital").each (function(){
                        //     this.reset();
                        // });
