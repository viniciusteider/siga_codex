...
$listar = $objEscalaMensal->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Data_hora_entrada];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Data_hora_saida];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["data_hora_entrada"], $_SESSION["usuario"]["id_fuso_horario"]),"class"=> "uppercase"];
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["data_hora_saida"], $_SESSION["usuario"]["id_fuso_horario"]),"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Escala_mensal;
//$filtro[ROTULO_RELATORIO] = Escala_mensal;
