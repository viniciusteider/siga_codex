<?php
class RelatorioQuantitativoAtendimento
{
    private $conexao;
    ...
    public function Gerar($idGrupo,$param)
    {
        ...
        FROM
          ...
          left JOIN `ocorrencias_recursos` ON (`ocorrencias`.`id` = `ocorrencias_recursos`.`id_ocorrencia`)
          left JOIN `recursos` ON (`recursos`.`id` = `ocorrencias_recursos`.`id_recurso`)
        ...
        $sql .= " GROUP BY ocorrencias.id_cidade,evento.id,subevento.id ,classificacao_risco.id ";
