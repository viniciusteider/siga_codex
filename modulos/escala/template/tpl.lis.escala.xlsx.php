...
$listar = $objEscala->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Data_fim];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["data_fim"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Escala;
//$filtro[ROTULO_RELATORIO] = Escala;
...
header("Content-Disposition: attachment;filename=".Escala.".xls");
