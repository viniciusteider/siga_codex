...
$listar = $objOcorrenciasMateriais->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Responsavel_hospital];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["responsavel_hospital"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Ocorrencias_materiais;
//$filtro[ROTULO_RELATORIO] = Ocorrencias_materiais;
...
header("Content-Disposition: attachment;filename=".Ocorrencias_materiais.".xls");
