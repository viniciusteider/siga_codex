...
$listar = $objFipeTipo->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem);

$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Id];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Nome];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => Nome_url];
...
if (@count($listar[0]) > 0) {
	foreach ($listar[0] as $linha) {
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["nome_url"],"class"=> "uppercase"];
...
$filtro[ROTULO_LISTAGEM] = Fipe_tipo;
//$filtro[ROTULO_RELATORIO] = Fipe_tipo;
...
header("Content-Disposition: attachment;filename=".Fipe_tipo.".xls");
