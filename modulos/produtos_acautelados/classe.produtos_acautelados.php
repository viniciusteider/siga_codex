...
class ProdutosAcautelados
{
	...
	public function Adicionar()
	{
		$pdo = $this->getConexao();
		$sql = '
		INSERT INTO produtos_acautelados SET data_hora_cadastro = UTC_TIMESTAMP() 
 ';
	...
	public function Modificar()
	{
		$pdo = $this->getConexao();
		$sql = '
		UPDATE produtos_acautelados SET 
			';
		 $sql .= ",id_produto = :id_produto";
		 $sql .= ",id_cautela = :id_cautela";
		 $sql .= ",quantidade = :quantidade";
	...
	public function Remover($lista)
	{
		$pdo = $this->getConexao();
		$lista = implode(",",$lista);
		//$sql = "DELETE FROM produtos_acautelados WHERE id IN({$lista})";
		$sql = "UPDATE produtos_acautelados SET excluido = UTC_TIMESTAMP() WHERE id IN({$lista})";
		$stmt = $pdo->prepare($sql);
		return $stmt->execute();
	}
	...
	public function ListarPaginacao($param)
	{
		...
		$where = "
			WHERE produtos_acautelados.excluido IS NULL
		";
		...
		if($param["busca"] != "") $where .= " AND (nome LIKE :busca)";
		 if (($param['data_hora_inicio']))  $where .= " AND produtos_acautelados.data_hora_cadastro >='{$param['data_hora_inicio']}' AND produtos_acautelados.data_hora_cadastro <= '{$param['data_hora_fim']}'";
		
		$sql = "
			SELECT COUNT(*) AS total
			FROM produtos_acautelados
			$joins
			$where
		";
		...
		$sql = "
			SELECT 
				produtos_acautelados.*
			FROM produtos_acautelados
			$joins
			$where
		";

		if($filtro != "") $sql .=" ORDER BY {$param['filtro']} {$param['ordem']}"; else $sql .=" ORDER BY produtos_acautelados.id DESC";
	...
	{
		$pdo = $this->getConexao();
		$sql = "SELECT * FROM produtos_acautelados WHERE id = :id";
		$stmt = $pdo->prepare($sql);
		$stmt->bindParam(":id",$this->id,PDO::PARAM_INT);
		$stmt->execute();
		return $stmt->fetch();
        ...
        {
            $pdo = $this->getConexao();
            $sql = "SELECT * FROM produtos_acautelados WHERE excluido IS NULL";
            if($id != "") $sql .= " AND produtos_acautelados.id = $id ";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        ...
        public function BuscarAutoComplete($busca){
            $pdo = new Conexao();
            $sql = " SELECT
                         produtos_acautelados.id,
                         produtos_acautelados.nome  AS text
                     FROM produtos_acautelados
                     WHERE produtos_acautelados.excluido IS NULL
                     ";
    
            if($busca != ''){
                $sql .= " AND  (produtos_acautelados.nome LIKE '%$busca%' OR produtos_acautelados.id  LIKE '%$busca%') ";
            }
            $sql .= ' LIMIT 50';
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        ...
        public function GerarSelec($id,$nome_campo,$id_campo,$outros = '',$campo = ["id","nome"],$autocomlete = false)
        {
            ...
            $registros = $objProdutosAcautelados->ListarCombo($id_atual);
