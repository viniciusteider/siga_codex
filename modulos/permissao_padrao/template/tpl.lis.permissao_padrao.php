<?php
include_once("modulos/permissao_padrao/template/js.lis.permissao_padrao.php");
$_SESSION["FILTRO_PERMISSAO_PADRAO"] = $_REQUEST;
...
$objPermissaoPadrao = new PermissaoPadrao();
$listar = $objPermissaoPadrao->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_permissao_padrao";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "PADRAO","filtro"=> "padrao", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["padrao"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["id"],"nome" => "Alterar","style" => "text-align:right"];
		$x++;
	}
}
//Componente::FiltrarRelatorioConfiguracao($dados_coluna, $dados_linha, $_SESSION["configuracao_usuario"]["permissao_padrao"]);
$grid = new GerarGrid();
$grid->form = $dados_form;
$grid->tabela = $dados_tabela;
$grid->titulo = "";
$grid->permitir_busca = false;
$grid->funcao_atualizar = "AtualizarGridPermissaoPadrao";
$grid->funcao_modificar = "ModificarPermissaoPadrao";
