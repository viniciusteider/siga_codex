<?php
include_once("modulos/paciente_medicamentos/template/js.lis.paciente_medicamentos.php");
$_SESSION["FILTRO_PACIENTE_MEDICAMENTOS"] = $_REQUEST;
...
$listar = $objPacienteMedicamentos->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_paciente_medicamentos";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "DOSE","filtro"=> "dose", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => Conexao::PrepararDataPHP($linha["horario"], $_SESSION["usuario"]["timezone"])];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["dose"]];
...
//Componente::FiltrarRelatorioConfiguracao($dados_coluna, $dados_linha, $_SESSION["configuracao_usuario"]["paciente_medicamentos"]);
$grid = new GerarGrid();
$grid->form = $dados_form;
$grid->tabela = $dados_tabela;
$grid->titulo = "";
$grid->permitir_busca = false;
$grid->funcao_atualizar = "AtualizarGridPacienteMedicamentos";
$grid->funcao_modificar = "ModificarPacienteMedicamentos";
$grid->valor_campo_busca = $busca;
$grid->filtro = $filtro;
$grid->id_botao_adicionar = "AdicionarRegistroPacienteMedicamentos";
$grid->id_botao_excluir = "ExcluirRegistroPacienteMedicamentos";
$grid->id_checkbox_master = "master_PacienteMedicamentos";
$grid->nome_lista_checkbox = "lista_PacienteMedicamentos";
$grid->pagina = $pagina;
