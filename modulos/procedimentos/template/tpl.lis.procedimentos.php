<?php
include_once("modulos/procedimentos/template/js.lis.procedimentos.php");
$_SESSION["FILTRO_PROCEDIMENTOS"] = $_REQUEST;
...
$listar = $objProcedimentos->ListarPaginacao($_SESSION['usuario']['id_grupo'],$numeroRegistros,$numeroInicioRegistro,$busca,$filtro,$ordem,$parametros);
...
$dados_tabela["id"]    = "id_tabela_procedimentos";
...
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "PROCEDIMENTO","filtro"=> "procedimento", "tipo"=> "$ordem"];
$dados_coluna["dados_th"][] = ["configuracao" => "", "nome" => "TIPO","filtro"=> "procedimento", "tipo"=> "$ordem"];
...
if(@count($listar[0])> 0){
	foreach($listar[0] as $linha){
		...
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["procedimento"]];
		$dados_linha[$x]["dados_td"][] = ["valor" => $linha["nome_tipo"]];
...
//Componente::FiltrarRelatorioConfiguracao($dados_coluna, $dados_linha, $_SESSION["configuracao_usuario"]["procedimentos"]);
$grid = new GerarGrid();
$grid->form = $dados_form;
$grid->tabela = $dados_tabela;
$grid->titulo = "";
$grid->permitir_busca = false;
$grid->funcao_atualizar = "AtualizarGridProcedimentos";
$grid->funcao_modificar = "ModificarProcedimentos";
