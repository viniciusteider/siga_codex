...
$(document).ready(function () {
    $("#bt_salvar").click(function () {
        ...
        if(id != "")
        {
            url = "index_xml.php?app_modulo=base&app_comando=atualizar_base&app_codigo";
            tipo = 2;
        }
        else
        {
            url = "index_xml.php?app_modulo=base&app_comando=adicionar_base&app_codigo";
        }
...
{
	    if (Squall.ValidateForm($("#frm_base"))) {
		...
		$.post(url,
			...
			function (response)
			{
				if (response["codigo"] == 0) {
                    Squall.ToastMsg('success',response["mensagem"],"#index_xml.php?app_modulo=base&app_comando=listar_base",'600');

                        // $("#frm_base").each (function(){
                        //     this.reset();
                        // });
